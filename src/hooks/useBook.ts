...
import { cartTableAtom, bookAtom, alertAtom } from '@/atoms'
    ...
    if(userAuth) { //사용자 로그인 유무 확인
      try {
        ...
        const isBook = data.some(
          (item: { id: number }) => item.id === Number(cartId)
        )
  ...
  const addBook = async () => {
    ...
    if(userAuth) {
        try {
          ...
          await axiosInstance.get(`/api/backend/append_interest/${userId}/${cartId}`) //개인AI에 데이터 추가 통신
  ...
  const deleteBook = async () => {
    ...
    if(userAuth) {
      try {
        ...
        await axiosInstance.get(`/api/backend/delete_interest/${userId}/${cartId}`) //개인AI에 데이터 삭제 통신
